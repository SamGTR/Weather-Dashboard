...
// Data persisting from local storage
for (var i = 0; i < localStorage.length; i++) {
    var cityName = localStorage.getItem(i);
    var buttonEl = document.createElement('button');
    buttonEl.textContent = cityName;
    buttonEl.setAttribute('class','cityButtonEl');
    buttonEl.setAttribute('value',cityName);
    divEl.appendChild(buttonEl);
}

// Use this for clearing local storage => localStorage.clear();
    ...
    if (cityName == ""){
        ...
        fetch(currentUrl)
            ...
            .then(function (data) { 
                ...
                // Condition for incorrect city name
                if (data.cod == "404"){
                    alert("Incorrect city name!");
                } else {

                    // Using last local storage key number
